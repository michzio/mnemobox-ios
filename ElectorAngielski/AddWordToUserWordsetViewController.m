//
//  AddWordToUserWordsetViewController.m
//  ElectorAngielski
//
...
#import "AddWordToUserWordsetViewController.h"
#import "WordCell.h"
...
//params: emailAddress, sha1Password, actionType, userWordsetId, translationId
#define kUSERWORDSET_EDITION_SERVICE_URL @"http://www.mnemobox.com/webservices/userWordsetEdition.php?email=%@&pass=%@&do=%@&uwid=%@&trans_id=%@"
#define kADD_ACTION @"add"
#define kDELETE_ACTION @"delete"

@interface AddWordToUserWordsetViewController()
...
@implementation AddWordToUserWordsetViewController
...
- (void) loadSuggestionsAsync: (BOOL) searchButtonTouched
{
    ...
    if(self.internetReachable.isReachable) {
        ...
        __weak AddWordToUserWordsetViewController *weakSelf = self;
...
- (UITableViewCell *) wordCellForTableView: (UITableView *)tableView andRowAtIndexPath: (NSIndexPath *) indexPath
{
    ...
    if([indexPath row] < [self.lookUpWords count]) {
        ...
        UIImage *recordingImage = [UIImage imageNamed:@"plus_icon.png"];
...
- (void) tableView: (UITableView *) tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
    ...
    WordObject *wordObject = [self.lookUpWords objectAtIndex: [indexPath row]];
    
    [self addToUserWordsetWord: wordObject];
    
    //[self performSegueWithIdentifier:@"Word Details Segue" sender:self];
}

- (void) addToUserWordsetWord: (WordObject *) wordObject
{
        NSLog(@"Adding word to user wordset in web server.");
        ...
        NSString *urlAsString = [NSString stringWithFormat:kUSERWORDSET_EDITION_SERVICE_URL, emailAddress, sha1Password, kADD_ACTION, idOfUserWordset, wordObject.wordId, nil];
        urlAsString = [urlAsString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
        NSLog(@"Add Word URL: %@", urlAsString);
